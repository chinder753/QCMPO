...
def directHmpo(h1e, h2e, partition=None, debug=False, iprt=0, isym=0):
    ...
    wdims = [1] + wdims + [1]
    wdims = np.array(wdims, dtype=np.int32)
    wfacs = [0] * ngroups
    qnums = [0] * ngroups
    if debug:
        print()
    ...
    for igroup in range(ngroups):
        ...
        a1 = np.array([0] + list(accumulate(dims1l)), dtype=np.int32)
        a2 = np.array([0] + list(accumulate(dims2l)), dtype=np.int32)
        if debug:
            print()
...
if __name__ == '__main__':
    ...
    def genRandomH(k):
        np.random.seed(1)
        h1e = np.random.uniform(-1, 1, size=(k, k))
        h2e = np.random.uniform(-1, 1, size=(k, k, k, k))
        h1e = 0.5 * (h1e + h1e.T)
